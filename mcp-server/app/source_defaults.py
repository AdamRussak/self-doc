...
MIRROR NOTE: this module is a deliberate, behaviorally-identical duplicate of
`ingestion/app/source_defaults.py`. `mcp-server` cannot import from
`ingestion` (separate deployables/venvs), hence the copy — see the note atop
`retrieval.py`'s `ProposedSourceConfig` for the same rationale applied to the
Pydantic model. Keep the two files in lock-step; a drift guard test
(`mcp-server/tests/test_server.py`) cross-imports `ingestion/app/
source_defaults.py` from disk and asserts `derive_name` agrees on a shared
fixture table of URLs.

Context (see `ingestion/app/urlscope.py` module docstring for the
include/exclude asymmetry this module must respect): a source config with
