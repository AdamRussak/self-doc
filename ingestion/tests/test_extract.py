...
def test_too_short_extraction_reports_length():
    # store.py's cross-page JS-shell detection (T6) needs the extracted
    # length even on a skipped/too-short result, so it can accumulate
    # lengths across a source's pages without re-parsing the HTML itself.
    html = "<html><body><p>Too short.</p></body></html>"
    result = extract("https://example.com/empty", html)
    assert result.status == "skipped"
    assert result.length == len("Too short.")


def test_ok_extraction_reports_length():
    html = """
    <html><body><article>
    <h1>Getting Started</h1>
    <p>This is a fairly long paragraph of real documentation content that should
