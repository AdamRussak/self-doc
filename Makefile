...
.PHONY: up down up-prod down-prod sync upload test test-db-up test-db-down test-db-reset build-cli test-cli install-cli install-skill install eval lint typecheck configure reindex backup backup-prune backup-auto restore purge refresh stop deploy-manifest
...
# Regenerate deploy/models.tsv, the machine-readable embedding-model manifest
# consumed by the deploy/ install kit's bash installer (a host assumed to
# have only docker/curl — no Python/PyYAML/jq, so it cannot read
# config/models.yaml directly). Generated by `scripts/models_matrix.py
# --format tsv` from that same config/models.yaml registry (see that
# script's module docstring for the exact pipe-delimited field contract,
# including why prompts are emitted RAW with any significant trailing
# space intact). Unlike db/init/01_schema.sql, a TSV cannot carry a
# "GENERATED, do not edit" comment header without corrupting the
# installer's `IFS='|' read -r` parse — so deploy/models.tsv itself has
# no such header; this comment is the only place that fact is recorded.
# The committed file must never drift from what this target produces: a
# parity test (mirroring the existing db/init/01_schema.sql parity guard
# in tests/test_model_registry.py) fails CI if it does. Re-run this
# target and commit the diff whenever config/models.yaml changes.
deploy-manifest:
	python3 scripts/models_matrix.py --format tsv > deploy/models.tsv
