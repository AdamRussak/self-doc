...
- **FastEmbed (BAAI/bge-small-en-v1.5)** for CPU-friendly ONNX embeddings *(Note: the model is no longer fixed — superseded by [ADR-004](004-selectable-embedding-models.md), which makes it registry-selectable. ADR-004's own default of `mixedbread-ai/mxbai-embed-large-v1` was later reverted — the registry default (`config/models.yaml`) is currently `BAAI/bge-small-en-v1.5` again, same as this original decision. See the status note atop ADR-004. FastEmbed/ONNX/CPU is unchanged.)*
