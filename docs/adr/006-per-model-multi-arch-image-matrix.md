...
the push-by-digest → `docker buildx imagetools create` merge is **documented
upstream behavior, but has not yet been observed on a real run of this
repo's `release.yml`.** The first real release under this workflow must be
verified with:

```bash
docker buildx imagetools inspect --raw ghcr.io/<owner>/self-docs-ingestion:<slug> \
  | jq '.manifests[] | {mediaType, platform}'
```

confirming the merged index lists **two platform image manifests** (amd64,
arm64) **plus their attestation manifests** (provenance + SBOM), not just the
two platform manifests alone. If attestations do not survive the merge, the
fallback is either dropping to plain `provenance: true` (no SBOM, less
metadata but simpler to reason about across the merge) or a separate
post-merge attestation-attach step.
